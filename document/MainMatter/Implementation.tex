...
\section{Aplicación para la captura de señales}
	...
	Se utlizaron las bibliotecas \textbf{sensors\textunderscore plus} y \textbf{geolocator}, disponibles en \textbf{Flutter}, para obtener acceso
	...
	\subsection{Metodología para marcar los baches}
		...
		bache, con el objetivo de minimizar el efecto del error inherente del \textbf{GPS}. Una vez que se terminaran de marcar los baches en el recorrido
...
\section{Los modelos de aprendizaje de máquinas}
	Luego de haber recopilado los datos con la aplicación móvil, estos fueron pasados a un programa hecho en \textbf{python} con el objetivo de llevar a cabo 
	...
	\subsection{Detalles de la detección de anomalías}
		...
		implementados en la biblioteca \textbf{sklearn} de python, específicamente en los módulos \textbf{sklearn.cluster}, donde se encuentran
	...
	\subsection{Detalles del proceso de etiquetado}
		Con cada uno de los juegos de datos obtenido por cada uno de los métodos utilizados para detectar anomalías, fue que se llevó a cabo
		...
		superficie de una esfera utilizando coordenadas de latitud-longitud. Debido a que la tierra no es una esfera perfecta, tiene distintos radios, que
	...
	\subsection{Detalles de la selección de modelo}
		...
		y de prueba se utilizó el método \textbf{train\textunderscore test\textunderscore split} del módulo \textbf{sklearn.model\textunderscore selection}.
		Del mismo módulo se utilizaron los objetos \textbf {RepeteadStratifiedKFold} y \textbf{GridSearchCV} para realizar validación cruzada y para
		selección de hiperparámetros respectivamente. De esta forma, para cada uno de los 5 métodos utilizados, se pudo determinar la mejor
		configuración de hiperparámetros, realizar la validación cruzada y obtener el mejor modelo para la fase de prueba.
		% ------------ Tabla con hiperparámetros para cada método ---------------- %
