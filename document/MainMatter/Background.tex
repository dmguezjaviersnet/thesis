...
Basado en este \textbf{umbral} T se convierte la imagen mejorada $I_{m}$ en una imagen binaria B que contiene el área defectuosa ~\brackcite{koch2011pothole}.
...
$T_b$ es el valor de \textbf{umbral} del método de Otsu \brackcite{jo2015pothole}.
...
Ryu et al. ~\brackcite{ryu2015image} primero proponen usar un ~\emph{median filter}. La idea principal del ~\emph{median filter} es recorrer la señal entrada 
por entrada, reemplazando cada entrada con la mediana de las entradas vecinas. Este filtro es utilizado para eliminar ruido como grietas o manchas. 
...
\subsection{Detección usando \emph{Convolutional Neural Network}}
	...
	excentricidad excede el \textbf{umbral} $\varepsilon+{max}$ y $\frac{l_{max}}{s}$ excede el \textbf{umbral} $r_{max}$ ~\brackcite{koch2011pothole}.
...
\section{Detección utilizando series temporales de otros sensores}
	...
	\subsection{Enfoques utilizando métodos heurísticos}
		...
		% y los procesaron con el objetivo de asignar los valores óptimos a cada uno de los \textbf{umbrales} característicos de cada uno de
		...
		% 		aceleración en un instante de tiempo determinado, que sea mayor que cierto \textbf{umbral} prefijado.\\
		% 	\item \textbf {v < 25 km/h}:  Utilizan un método que llamaron \textbf {Z-SUS} que busca una caída sostenida en la aceleración
		% 		en el eje Z, por debajo de un cierto \textbf{umbral} prefijado, durante cierta cantidad de tiempo.\\
		% \end{itemize}


		El algoritmo para la detección de \textbf{anomalías} en la carretera propuesto por \emph{Eriksson et al.} ~\brackcite{eriksson2008pothole} sigue
	...
	\subsection{Enfoques utilizando \emph{Machine Learning}}
		...
		\subsubsection{Features}
			...
			pawar2020efficient}, incluso \textbf{características} como cuantas veces cierta \textbf{característica} estadística sobrepasó cierto \textbf
			{umbral} establecido \brackcite{carlos2018evaluation}, una idea basada en los primeros trabajos en relación a este problema y que utilizaron
			...
			perttunen2011distributed}, otras fueron extraídos utilizando \emph{Wavelet Decomposition} específicamente \emph{Stationary Wavelet
			...
			o \emph{Principal Components Analysis}(\textbf{PCA}) \brackcite {cong2013applying}. \emph{gonzalez2017learning} usan las \textbf{características}
		...
		\subsubsection{Etiquetado de los datos}
			...
			{el2018towards}. \brackcite{gonzalez2017learning} etiquetaron las \textbf{anomalías} de la siguiente forma, cuando se van acercando a la anomalía
			...
			\textbf{serie temporal}. \brackcite{tai2010automatic} etiquetaron sus datos realizando grabaciones de audio durante el viaje recolectando la voz 
	...
	\subsection{Purificación y segmentación de la señal}
		...
		{el2018towards} y \emph{Wavelet Discrete Transform} \brackcite{seraj2015roads}, y \emph{high-pass filters} y \emph{low-pass filters} para eliminar
		componentes de baja y alta frecuencia respectivamente \brackcite{eriksson2008pothole, kulkarni2014pothole}. También es importante determinar de que
		forma segmentar la señal para su análisis, hay propuestas que segmentan la señal en 256 ventanas \brackcite{eriksson2008pothole}, en ventanas con
		duración de 1 segundo \brackcite{el2018towards}, ventanas deslizantes con duración de de 2.5 segundos con el 66\% de las ventanas solapándose
		\brackcite{seraj2015roads}, ventanas deslizantes con varias duraciones entre 0.5 y 2 segundos \brackcite{perttunen2011distributed} y una interesante
	...
	\subsection{Geolocalización del bache utilizando GPS}
		...
		valor \textbf{umbral}. Esta idea fue tomada teniendo en cuenta una serie de artículos con respecto al tema(~\brackcite{mednis2011real}), con el
		...
		~\brackcite{seraj2015roads} en su propuesta, incorporaron además los datos del giroscopio con el objetivo de obtener más \textbf{características}
		...
		~\brackcite{cong2013applying} para la extracción de \textbf{características} descomponen la señal usando \textbf{WPD}(\emph{Wavelet Packet Decomposition})
